	...
	function generate()
	{
		...
		if(count($page_list)) {
			foreach($page_list as $page) {
				...
				if(!$this->add_url_element($page_url,  date('c', strtotime($page->updated_at?$page->updated_at:$page->created_at)), 'weekly', '0.7'))
 					break;
 			}
		}
		...
		if($this->url_count < self::max_urls && $this->params->include_categories) {
			...
			foreach($category_list as $category) {
				...
				if(!$this->add_url_element($category_url,  date('c', strtotime($category->updated_at?$category->updated_at:$category->created_at)), $this->params->categories_changefreq, $this->params->categories_priority))
 					break;
 			}
		}
			...
			if($lssalestracking_installed && class_exists('LsSalesTracking_ProductManager')) {
 			...
 			foreach($product_list as $product) {
 				...
 				if(substr($product_url, -1) != '/') $product_url .= '/';
	 				if(!$this->add_url_element($product_url,  date('c', strtotime($product->updated_at?$product->updated_at:$product->created_at)), $this->params->products_changefreq, $this->params->products_priority))
	 					break;
	 			}
		}
		if($this->url_count < self::max_urls && $this->params->include_blogposts) {
 			...
 			foreach($blog_post_list as $blog_post) {
 				...
 				if(!$this->add_url_element($blogpost_url,  date('c', strtotime($blog_post->published_date)), $this->params->blogposts_changefreq, $this->params->blogposts_priority))
 					break;
 			}
		}
		...
		if($this->url_count < self::max_urls && $wiki_installed && $this->params->include_wiki && class_exists('Wiki_Page')) {
			...
			foreach($wiki_page_list as $wiki_page) {
				...
				if(!$this->add_url_element($wiki_url, date('c', strtotime($wiki_page->updated_at?$wiki_page->updated_at:$wiki_page->created_at)), $this->params->wiki_changefreq, $this->params->wiki_priority))
					break;
			}
		} 			
